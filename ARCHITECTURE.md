...
> `client/public/{favicon,logo,logo-daylight}.svg` (la couleur de marque #90A4AE est posée dans `Header.tsx`)
...
| **M3 → M12 + F1 → F8** | ✅ **CONSTRUITS** | 28 migrations, 37 pages, `shared` + `server` + `client` compilent proprement. Cinq audits adversariaux, 24 défauts critiques fermés. Recette faite contre des faux protocolaires et des fixtures — jamais contre du matériel. |
| M13 — i18n | ✅ **LIVRÉ** | Mesure faite plutôt que supposée : les 18 locales étaient déjà structurellement complètes et traduites à 94-98 %. Le trou réel était 24 chaînes codées en dur dans des composants (dont une en FRANÇAIS dans `SsoEnrollPage`), invisibles à un diff de clés. Toutes extraites, traduites dans les 18 langues, 2124 clés identiques partout, aucun littéral restant. |
| **Chemin d'écriture 3 marques** | 🔄 **PARTIEL** | `assertTargetBinding` aiguille MikroTik (API RouterOS) et DrayTek/Zyxel (SSH via le driver) ; **SonicOS REST refusé explicitement**. `applyOverSsh` écrit, jamais exécuté. `PEER_RECOVERY_ADAPTERS` volontairement vide — voir 7.x. |
| **Critère de bruit M4** | ⬜ **JAMAIS MESURÉ** | « Moins de 3 findings de bruit par équipement. » Aucun faux ne peut le produire ; il demande le parc réel. |
...
| **Identité visuelle** (favicon, wordmark clair/sombre, couleur de marque) | À la charge de Vaintor. En attendant, `client/public/{favicon,logo,logo-daylight}.svg` gardent l'artwork Obliguard et la couleur de marque **#90A4AE est arbitrée et posée** dans `Header.tsx`. Les trois SVG restent des **placeholders assumés**, ne pas les signaler comme bug. |
...
| **M14 — relais pour sites étanches : DÉLIBÉRÉMENT NON CONSTRUIT** | Arbitré après conception complète (4 approches, 3 lentilles adversariales). Le besoin est réel : certains clients ont un réseau multisite quasi étanche entre leur DC et leurs boutiques, majoritairement DrayTek et SonicWall, sans session CHR ni client TR-069. Mais **le tunnel doit remonter jusqu'à ObliWAN, pas le logiciel descendre chez le client.** Trois formes, par ordre de préférence : (F1) la boutique compose elle-même un L2TP/IPsec vers le CHR — MikroTik et DrayTek Vigor savent le faire en dial-out, et rien du produit ne change ; (F2) le SonicWall **initie un IKEv2 vers le CHR**, qui répond — SonicOS n'a pas de client L2TP site-à-site mais sait faire ça ; il faut alors `tunnel_sessions` en miroir de `ppp_sessions`, `devices.peer_identity` (l'identité IKE, prouvée par le secret à l'établissement, **jamais une adresse**) et une vue `presence_sessions` unissant les deux ; (F3) la tête de réseau route — représentable sans migration, irréductiblement dégradé, **lecture seule**. **Refusé sans contre-proposition : le relais appairé sortant** (le « proxy middleware » au sens littéral). Trois défauts de conception, pas des bugs : l'intermédiaire choisit la destination (le message d'ouverture porte `{device_id, transport}` et le relais résout l'hôte dans *sa* copie de la politique — D5/R4 inversé, `assertTargetBinding` s'exécutant sur un canal que le relais a lui-même câblé) ; le coffre se duplique chez le client ; aucun diagnostic hors-bande par construction, R5 violé là où le produit l'exige. Noté 3/3/2 sur les trois lentilles contre 8,5/8/8 pour l'extension réseau, pour 28 à 36 semaines réelles achetant de la **lecture seule**. Refusés aussi : un sixième `TransportKind` (98 références dans 23 fichiers ; « je suis relayé » n'est pas une propriété de la boîte), le rebond SSH (ni DrayTek ni SonicOS ne savent forwarder, et le rebond ne porte pas l'UDP donc ni SNMP ni syslog), le provisionnement par DHCP option 43. **À réexaminer si une marque sans slot SIM et sans TR-069 entre au catalogue.** |
