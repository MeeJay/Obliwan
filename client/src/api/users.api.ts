...
export const usersApi = {
  /**
   * `scope=all` is honoured by the server for a PLATFORM ADMIN only; every other
   * caller gets the tenant-scoped list regardless. Without it the admin screen
   * showed only members of the current tenant, so a user created by POST /users
   * — which creates no user_tenants row — vanished from the very screen used to
   * assign them a tenant. Passing it is safe: it widens nothing for a tenant admin.
   */
  async list(): Promise<User[]> {
    const res = await apiClient.get<ApiResponse<User[]>>('/users', { params: { scope: 'all' } });
