...
// Exportable entities. Spec §1.4 bis: every exported entity must be
// re-importable — no orphans. Inventory, templates, revisions and variables
// join this list as their milestones land (M2, M5).
type Section =
  | 'deviceGroups'
  | 'settings'
...
function SectionSelector({
  ...
  const SECTION_LABELS: Record<Section, string> = {
    deviceGroups:        t('importExport.deviceGroups'),
...
export function ImportExportPage() {
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      a.href     = url;
      a.download = `obliwan-export-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
  ...
  const processFile = useCallback((file: File) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        toast.error('Invalid export file — expected ObliWAN JSON export');
        setImportFile(null);
        setImportData(null);
  ...
  return (
    <div className="min-w-0 px-4 py-8">
      ...
      <div className="mb-8">
        ...
        <a
          href="/obliwan-import-example.json"
          download="obliwan-import-example.json"
