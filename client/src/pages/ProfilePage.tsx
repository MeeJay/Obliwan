...
export function ProfilePage() {
  ...
  // Removing a second factor now requires proving account ownership server-side:
  // a stolen session must not be enough. The button opens this prompt instead of
  // firing the request, so the 400 the server would return never reaches the user.
  const [disarm, setDisarm] = useState<null | { factor: 'totp' | 'email'; kind: 'password' | 'code' }>(null);
              ...
              <div className="flex items-center justify-between">
                ...
                {tfaStatus?.totpEnabled ? (
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => { setDisarmProof(''); setDisarm({ factor: 'totp', kind: 'password' }); }}
                  >
                    {t('common.disable')}
                  </Button>
                ) : !totpSetupData ? (
              ...
              <div className="flex items-center justify-between">
                ...
                {tfaStatus?.emailOtpEnabled ? (
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => { setDisarmProof(''); setDisarm({ factor: 'email', kind: 'password' }); }}
                  >
                    {t('common.disable')}
                  </Button>
                ) : emailSetupStep === 'idle' ? (
              ...
              {disarm && (
                <div className="space-y-3 rounded-md border border-amber-500/40 bg-amber-500/5 p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-text-primary">{t('profile.security.disableTitle')}</p>
                      <p className="text-xs text-text-secondary">{t('profile.security.disableDesc')}</p>
                    </div>
                  </div>

                  <Input
                    type={disarm.kind === 'password' ? 'password' : 'text'}
                    label={disarm.kind === 'password' ? t('profile.password.current') : t('profile.security.verificationCode')}
                    value={disarmProof}
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button
                      size="sm"
                      disabled={disarmBusy || disarmProof.trim() === ''}
                      onClick={async () => {
                        setDisarmBusy(true);
                        // Exactly one of the two: the server tries currentPassword first
                        // and fails hard if it is wrong, so sending both would make the
                        // code branch unreachable for anyone with a local password.
                        const proof = disarm.kind === 'password'
                          ? { currentPassword: disarmProof }
                          : { code: disarmProof.trim() };
                        try {
                          if (disarm.factor === 'totp') {
                          ...
                          // Surface the server's reason: it distinguishes a wrong password
                          // from an invalid code from a missing proof, and the user needs
                          // to know which one it was.
                          const msg = (err as { response?: { data?: { error?: string } } })?.response?.data?.error;
                          toast.error(msg ?? t(disarm.factor === 'totp'
                            ? 'profile.security.failedDisableTotp'
                            : 'profile.security.failedDisableEmailOtp'));
                        } finally {
                          setDisarmBusy(false);
                        }
                      }}
                    ...
                    <Button size="sm" variant="ghost" onClick={() => { setDisarm(null); setDisarmProof(''); }}>
                      {t('common.cancel')}
                    </Button>

                    {/* An SSO-only account has no local password: its enrolled factor is
                        the only proof it can offer. */}
                    <button
                      type="button"
                      className="ml-auto text-xs text-text-secondary underline hover:text-text-primary"
                      onClick={() => {
                        setDisarmProof('');
                        setDisarm({ ...disarm, kind: disarm.kind === 'password' ? 'code' : 'password' });
                      }}
                    >
                      {disarm.kind === 'password'
                        ? t('profile.security.proofWithCode')
                        : t('profile.security.proofWithPassword')}
                    </button>
                  </div>
                </div>
              )}
