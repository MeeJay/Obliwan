...
function buildAdaptiveCard(payload: NotificationPayload): Record<string, unknown> {
  ...
  const title = payload.isGroupNotification
    ...
    : `${emoji} ${payload.entityName}`;

  const subtitle = payload.isGroupNotification
    ? `${payload.downMembers?.length ?? 0} monitor(s) affected`
  ...
  if (payload.entityUrl) {
    facts.push({ title: 'URL', value: payload.entityUrl });
  }
  ...
  if (payload.isGroupNotification && payload.downMembers?.length) {
    facts.push({ title: 'Affected monitors', value: payload.downMembers.join(', ') });
  }
